-- INVESTIGATE AND FIX THE CONSTRAINT ISSUE
-- The problem: ON CONFLICT (user_id) but constraint has auto-generated name

-- Step 1: See what constraints actually exist
SELECT 'ACTUAL CONSTRAINTS ON profiles TABLE:' as info;
SELECT 
    tc.constraint_name,
    tc.constraint_type,
...
-- Step 2: Show the actual table structure
...
-- Step 3: Drop the current problematic function
DROP FUNCTION IF EXISTS public.add_user_to_group(uuid, uuid) CASCADE;

-- Step 4: Create a function that uses the CORRECT constraint name
...
BEGIN
  ...
  RETURN jsonb_build_object(
    ...
    'constraint_name_found', constraint_name
  );
...
BEGIN
  ...
  RETURN jsonb_build_object(
    ...
    'status', CASE 
      WHEN (test_result->>'success')::boolean THEN 'CONSTRAINT ISSUE FIXED' 
      ELSE 'STILL BROKEN - CHECK ERROR' 
    END
  );
END;
$$;

GRANT EXECUTE ON FUNCTION public.test_constraint_fix() TO authenticated, anon, service_role;

SELECT 'INVESTIGATION AND FIX APPLIED - Test with: SELECT public.test_constraint_fix();' as status;
